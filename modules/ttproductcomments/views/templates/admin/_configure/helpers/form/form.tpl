    ...
    {if $input.type == 'products'}
        <table id="{$input.name}">
            <tr>
                <th></th>
                <th>ID</th>
                <th width="80%">{l s='Product Name' mod='ttproductcomments'}</th>
            </tr>
        ...
        {foreach $input.values as $value}
            ...
            <label class="t" for="{$value.id}">
                {if isset($input.is_bool) && $input.is_bool == true}
                    {if $value.value == 1}
                        <img src="../img/admin/enabled.gif" alt="{$value.label}" title="{$value.label}" loading="lazy"/>
                    {else}
                        <img src="../img/admin/disabled.gif" alt="{$value.label}" title="{$value.label}" loading="lazy"/>
                    {/if}
                {else}
                    {$value.label}
                {/if}
