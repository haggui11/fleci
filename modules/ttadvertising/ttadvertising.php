...
class TtAdvertising extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('TT - Advertising block', array(), 'Modules.Advertising.Admin');
        ...
        if ($displayLeftColumn || $displayRightColumn) {
            ...
            foreach (scandir(_PS_MODULE_DIR_.$this->name) as $file) {
                if (in_array($file, array('advertising.jpg', 'advertising.gif', 'advertising.png'))) {
                    Configuration::updateGlobalValue('BLOCKADVERT_IMG_EXT', Tools::substr($file, strrpos($file, '.') + 1));
                }
            }
        ...
        if (Tools::isSubmit('submitAdvConf')) {
            if (isset($_FILES['adv_img']) && isset($_FILES['adv_img']['tmp_name']) && !empty($_FILES['adv_img']['tmp_name'])) {
                if ($error = ImageManager::validateUpload($_FILES['adv_img'], Tools::convertBytes(ini_get('upload_max_filesize')))) {
                    ...
                    Configuration::updateValue('BLOCKADVERT_IMG_EXT', Tools::substr($_FILES['adv_img']['name'], strrpos($_FILES['adv_img']['name'], '.') + 1));
            ...
            $this->initialize();
            $this->_clearCache('ttadvertising');
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch(
            'module:ttadvertising/ttadvertising.tpl',
            $this->getCacheId('ttadvertising')
        );
    }
}
