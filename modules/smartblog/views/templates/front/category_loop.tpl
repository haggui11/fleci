...
<div itemtype="#" itemscope="" class="sdsarticleCat clearfix">
	<div id="smartblogpost-{$post.id_post}">
		...
		<div class="articleContent col-sm-5 col-xs-5">
			<a href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}" title="{$post.meta_title}" class="imageFeaturedLink">
				{assign var="activeimgincat" value='0'}
				{$activeimgincat = $smartshownoimg} 
				{if ($post.post_img != "no" && $activeimgincat == 0) || $activeimgincat == 1}
					<img alt="{$post.meta_title}" src="{$link->getMediaLink($smart_module_dir)}/smartblog/views/img/{$post.post_img}-single-default.jpg" class="imageFeatured" loading="lazy">
				{/if}
			</a>
		</div>
		...
		<span class="blogdetail">
			...
			{if $smartshowauthor ==1}&nbsp;
				<span class="author"><i class="material-icons user">&#xE7FF;</i>&nbsp;&nbsp;{if $smartshowauthorstyle != 0}{$post.firstname} {$post.lastname}{else}{$post.lastname} {$post.firstname}{/if}</span>&nbsp;&nbsp;
			{/if}
			<span class="articleSection"><a href="{smartblog::GetSmartBlogLink('smartblog_category',$catlink)}"><i class="material-icons tags">&#xE54E;</i>&nbsp;&nbsp;{if $title_category != ''}{$title_category}{else}{$post.cat_name}{/if}</a></span>&nbsp;&nbsp;
