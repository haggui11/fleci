...
class AdminBlogPostController extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->lang = true;
        $this->image_dir = '../modules/smartblog/views/img';
    ...
    public function processImage($FILES, $id)
    {
        if (isset($FILES['image']) && isset($FILES['image']['tmp_name']) && !empty($FILES['image']['tmp_name'])) {
            if (ImageManager::validateUpload($FILES['image'], 4000000)) {
                ...
                $path = _PS_MODULE_DIR_ .'smartblog/views/img/' . $file_name;
                ...
                if (!move_uploaded_file($FILES['image']['tmp_name'], $path)) {
                    ...
                    foreach ($posts_types as $image_type) {
                        $dir = _PS_MODULE_DIR_ .'smartblog/views/img/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg';
                        if (file_exists($dir)) {
                            unlink($dir);
                        }
                    }
                    foreach ($posts_types as $image_type) {
                        ImageManager::resize($path, _PS_MODULE_DIR_ .'smartblog/views/img/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg', (int)$image_type['width'], (int)$image_type['height']);
                    }
                }
            }
        }
    ...
    public function renderForm()
    {
        ...
        if (Tools::getvalue('id_smart_blog_post') != '' && Tools::getvalue('id_smart_blog_post') != null) {
            $img_desc .= '<br/><img style="height:auto;width:300px;clear:both;border:1px solid black;" alt="" src="'.__PS_BASE_URI__.'modules/smartblog/views/img/'.Tools::getvalue('id_smart_blog_post').'.jpg" /><br />';
        }
