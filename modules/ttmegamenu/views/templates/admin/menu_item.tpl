...
<form id="module_form" class="defaultForm form-horizontal" action="index.php?controller=AdminModules&amp;configure=ttmegamenu&amp;token={Tools::getAdminTokenLite('AdminModules')|escape:'html':'UTF-8'}" method="post" enctype="multipart/form-data" novalidate="">
<div class="panel"><h3><i class="icon-list-ul"></i> {l s='Menu Item' mod='ttmegamenu'}</h3>
	<div class="form-wrapper" id="menuContent" >
		<div class="form-group tt-type-link">
			<label class="control-label col-lg-3">{l s='Type Link' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				<div class="radio tt-radio">
		...
		<div class="form-group tt-menu-title" {if $menu->type_link == 0}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='Title' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
		...
		<div class="form-group tt-menu-link" {if $menu->type_link == 0}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='Link' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					{if $languages|count > 1}
						<div class="translatable-field lang-{$language.id_lang|intval}" {if $language.id_lang != $id_language}style="display:none"{/if}>
		...
		<div class="form-group ps_link" {if $menu->type_link == 1}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='PrestaShop Link' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
		...
		<div class="form-group show_sub" {if $menu->type_link == 1}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='Show Sub Categories' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
		...
		<div class="form-group">
			<label class="control-label col-lg-3">{l s='Sub Title' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					{if $languages|count > 1}
						<div class="translatable-field lang-{$language.id_lang|intval}" {if $language.id_lang != $id_language}style="display:none"{/if}>
		...
		<div class="form-group tt-type-icon">
			<label class="control-label col-lg-3">{l s='Type Icon' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				<div class="radio tt-radio">
		...
		<div class="form-group tt-fw-icon" {if $menu->type_icon == 0}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='Material Icons Icon' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				...
				<p>{l s='Put class icon of Material Icons at :' mod='ttmegamenu'} <a href="https://material.io/tools/icons/?style=baseline">https://material.io/tools/icons/?style=baseline.</a> ex: icon-sun</p>
			</div>
		</div>
		<div class="form-group tt-img-icon" {if $menu->type_icon == 1}style="display:none"{/if}>
			<label class="control-label col-lg-3">{l s='Image Icon' mod='ttmegamenu'}</label>
			<div class="col-lg-9">
				<div class="col-lg-6">
					...
					<div class="dummyfile input-group">
						...
						<span class="input-group-btn">
							<button id="icon_img-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
								<i class="icon-folder-open"></i> {l s='Add file' mod='ttmegamenu'}				</button>
										</span>
					</div>
					{if $menu->type_icon == 0 && isset($menu->icon) && $menu->icon != ''}
						<img src="{$image_baseurl|escape:'html':'UTF-8'}{$menu->icon|escape:'html':'UTF-8'}" class="img-thumbnail" loading="lazy"/>
						{if isset($menu->id)}
							<a href="index.php?controller=AdminModules&amp;configure=ttmegamenu&amp;tab_module=front_office_features&amp;module_name=ttmegamenu&amp;token={Tools::getAdminTokenLite('AdminModules')|escape:'html':'UTF-8'}&amp;removeIcon=1&amp;id_ttmegamenu={$menu->id|intval}" id="del_icon">Remove</a>
						{/if}
					{/if}
						<script type="text/javascript">
						$(document).ready(function(){
	...
	<div class="form-group">
		...
		<div class="col-lg-9">
			<select class="form-control fixed-width-lg" name="align_sub" id="align_sub">
				<option value="tt-sub-left">{l s='Left' mod='ttmegamenu'}</option>
				<option value="tt-sub-right">{l s='Right' mod='ttmegamenu'}</option>
				<option value="tt-sub-center">{l s='Center' mod='ttmegamenu'}</option>
				<option value="tt-sub-auto">{l s='Auto' mod='ttmegamenu'}</option>
			</select>
			<script type="text/javascript">
				$(document).ready(function(){
	...
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Width Of SubMenu' mod='ttmegamenu'}</label>
		<div class="col-lg-9">
	...
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Class' mod='ttmegamenu'}</label>
		<div class="col-lg-9">
			<input type="text" class="class" id="class" name="class" value="{if $menu->class}{$menu->class|escape:'html':'UTF-8'}{/if}"/>
		</div>
	</div>
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Active' mod='ttmegamenu'}</label>
		<div class="col-lg-9">
			<span class="switch prestashop-switch fixed-width-lg">
	...
	<div class="panel-footer" style="margin-bottom:10px;">
		<input type="hidden" name="id_ttmegamenu" id="id_ttmegamenu" value="{if isset($menu->id)}{$menu->id|intval}{/if}"/>
