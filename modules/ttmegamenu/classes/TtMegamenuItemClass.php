...
class TtMegamenuItemClass extends ObjectModel
{
    public $id_column;
    public $type_link;
    public $type_item;
    ...
    public function __construct($id_item = null, $id_lang = null, $id_shop = null, Context $context = null)
    {
        ...
        Shop::addTableAssociation('ttmegamenu_item', array(
            'type' => 'shop'
        ));
        Shop::addTableAssociation('ttmegamenu_item_lang', array(
            'type' => 'fk_shop'
        ));
        if ($this->id) {
    ...
    public function getFieldShop($field)
    {
        $id_shop = (int)Context::getContext()
            ->shop->id;
        $sql = 'SELECT wms.' . $field . ' FROM ' . _DB_PREFIX_ . 'ttmegamenu_item wm
		LEFT JOIN ' . _DB_PREFIX_ . 'ttmegamenu_item_shop wms ON (wm.id_item = wms.id_item)
    ...
    public function getPositionByMenuItem($id_menu_item)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)
            ->getRow('
			SELECT ms.`position`
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_item_shop ms
