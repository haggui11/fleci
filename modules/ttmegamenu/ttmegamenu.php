...
include_once(_PS_MODULE_DIR_ . 'ttmegamenu/classes/TtMegamenuClass.php');
include_once(_PS_MODULE_DIR_ . 'ttmegamenu/classes/TtMegamenuRowClass.php');
include_once(_PS_MODULE_DIR_ . 'ttmegamenu/classes/TtMegamenuColumnClass.php');
include_once(_PS_MODULE_DIR_ . 'ttmegamenu/classes/TtMegamenuItemClass.php');
include_once(_PS_MODULE_DIR_ . 'ttmegamenu/sql/SampleDataMenu.php');

class Ttmegamenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()
            ->trans('TT - Megamenu', array(), 'Modules.TtMegamenu.Admin');
        $this->description = $this->getTranslator()
            ->trans('Show Megamenus on your site', array(), 'Modules.TtMegamenu.Admin');
    ...
    public function getContent()
    {
        ...
        } elseif ((Tools::isSubmit('buildMenu') || Tools::isSubmit('submitRow') || Tools::isSubmit('delete_row')) && Tools::isSubmit('id_ttmegamenu') || Tools::isSubmit('changeStatusRow') || Tools::isSubmit('submitCol') || Tools::isSubmit('delete_col') || Tools::isSubmit('submitSubItem') || Tools::isSubmit('delete_submenu_item') || Tools::isSubmit('changeStatusSubItem')) {
            $this->_postProcess();
        ...
        } elseif (Tools::isSubmit('addMenu') || (Tools::isSubmit('id_ttmegamenu') && $this->menuExists(Tools::getValue('id_ttmegamenu')))) {
    ...
    public function renderList()
    {
        ...
        foreach ($info_menus as $key => $info_menu) {
            $info_menus[$key]['status'] = $this->displayStatus($info_menu['id_ttmegamenu'], $info_menu['active']);
    ...
    public function getMenuInfo($active = null)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)
            ->executeS('
			SELECT pc.*, pl.*
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_shop pc
			LEFT JOIN ' . _DB_PREFIX_ . 'ttmegamenu_lang pl ON (pc.id_ttmegamenu = pl.id_ttmegamenu AND pc.`id_shop` = pl.`id_shop`)
			WHERE pl.id_shop = ' . $id_shop . ' AND pl.id_lang = ' . $id_lang . ($active ? ' AND pc.`active` = 1' : ' ') . ' ORDER BY pc.position ASC, pc.id_ttmegamenu ASC');
    }

    public function displayStatus($id_ttmegamenu, $active)
    {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitMenuItem')) {
            $this->_clearCache('ttmegamenu.tpl');
            if (Tools::getValue('id_ttmegamenu')) {
                $menu_item = new TtMegamenuClass((int)Tools::getValue('id_ttmegamenu'));
                $pos_arr = $menu_item->getPositionByMenu((int)Tools::getValue('id_ttmegamenu'));
                $menu_item->position = $pos_arr['position'];
                if (!Validate::isLoadedObject($menu_item)) {
                    $this->html .= $this->displayError($this->l('Invalid id_ttmegamenu'));
                    return false;
                }
            } else {
                $menu_item = new TtMegamenuClass();
            ...
            return $errors;
        } elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_ttmegamenu')) {
            $this->_clearCache('ttmegamenu.tpl');
            $menu = new TtMegamenuClass((int)Tools::getValue('id_ttmegamenu'));
            if ($menu->active == 0) {
                $menu->active = 1;
            } else {
                $menu->active = 0;
            ...
            $pos_arr = $menu->getPositionByMenu((int)Tools::getValue('id_ttmegamenu'));
        ...
        } elseif (Tools::isSubmit('removeIcon') && Tools::isSubmit('id_ttmegamenu')) {
            $this->_clearCache('ttmegamenu.tpl');
            $menu = new TtMegamenuClass((int)Tools::getValue('id_ttmegamenu'));
            $icon = $menu->icon;
            if (preg_match('/sample/', $icon) === 0) {
                if ($icon && file_exists(_PS_MODULE_DIR_ . 'ttmegamenu/views/img/icons/' . $icon)) {
                    @unlink(_PS_MODULE_DIR_ . 'ttmegamenu/views/img/icons/' . $icon);
                }
            }
            $menu->icon = '';
            $menu->update();
            ...
            if (Tools::getValue('id_row')) {
                $row_item = new TtMegamenuRowClass(Tools::getValue('id_row'));
                if (!Validate::isLoadedObject($row_item)) {
                    $this->html .= $this->displayError($this->l('Invalid id_row'));
                    return false;
                }
            } else {
                $row_item = new TtMegamenuRowClass();
            }
            $row_item->active = (int)Tools::getValue('active');
            $row_item->id_ttmegamenu = (int)Tools::getValue('id_ttmegamenu');
            ...
            $this->_clearCache('ttmegamenu.tpl');
            $menu_item = new TtMegamenuClass((int)Tools::getValue('delete_id_menu'));
            ...
            if (!$res) {
                ...
                ->context
                ->link
                ->getAdminLink('AdminModules', true) . '&conf=1&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_ttmegamenu=' . Tools::getValue('id_ttmegamenu'));
            }
        } elseif (Tools::isSubmit('delete_row')) {
            $this->_clearCache('ttmegamenu.tpl');
            $row_item = new TtMegamenuRowClass((int)Tools::getValue('id_row'));
            $res = $row_item->delete();
            if (!$res) {
                $this->html .= $this->displayError('Could not delete.');
            } else {
                ...
                ->context
                ->link
                ->getAdminLink('AdminModules', true) . '&conf=1&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_ttmegamenu=' . Tools::getValue('id_ttmegamenu') . '&buildMenu=1');
            }
            ...
            $id_shop = (int)$this
                ->context
                ->shop->id;
            $row = new TtMegamenuRowClass((int)Tools::getValue('id_row'), null, $id_shop);
            if ($row->active == 0) {
                $row->active = 1;
            } else {
                $row->active = 0;
            ...
            if (Tools::getValue('id_column')) {
                $col_item = new TtMegamenuColumnClass(Tools::getValue('id_column'));
                ...
                $col_item = new TtMegamenuColumnClass();
            ...
            $this->_clearCache('ttmegamenu.tpl');
            $col_item = new TtMegamenuColumnClass((int)Tools::getValue('id_column'));
            $res = $col_item->delete();
            if (!$res) {
                $this->html .= $this->displayError('Could not delete.');
            } else {
            ...
            if (Tools::getValue('id_item')) {
                $sub_menu_item = new TtMegamenuItemClass(Tools::getValue('id_item'));
                ...
                $sub_menu_item = new TtMegamenuItemClass();
            ...
            $this->_clearCache('ttmegamenu.tpl');
            $submenu_item = new TtMegamenuItemClass((int)Tools::getValue('id_item'));
    ...
    public function renderAddForm()
    {
        ...
        $id_ttmegamenu = Tools::getValue('id_ttmegamenu');
        if (isset($id_ttmegamenu)) {
            $menu = new TtMegamenuClass($id_ttmegamenu);
        } else {
            $menu = new TtMegamenuClass();
        }
 ...
 //       $id_column = Tools::getValue('id_column');
        ...
        $fields_form = array(
            'form' => array(
                ...
                'buttons' => array(
                    array(
                        'href' => AdminController::$currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&id_ttmegamenu=' . $id_ttmegamenu . '&buildMenu=1',
                        'title' => $this->l('Cancel') ,
        ...
        if (Tools::isSubmit('id_item')) {
            ...
            $subMenu_item = new TtMegamenuItemClass(Tools::getValue('id_item'));
        ...
        $helper->currentIndex = $this
            ->context
            ->link
            ->getAdminLink('AdminModules', false) . '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_ttmegamenu=' . $id_ttmegamenu . '&buildMenu=1';
    ...
    public function getAddFieldsValuesLink()
    {
        ...
        $languages = Language::getLanguages(false);
        if (Tools::isSubmit('id_item')) {
            $menu_item = new TtMegamenuItemClass((int)Tools::getValue('id_item'));
            ...
            $menu_item = new TtMegamenuItemClass();
...
//        $languages = Language::getLanguages(false);
        if (Tools::isSubmit('id_row')) {
            $row = new TtMegamenuRowClass((int)Tools::getValue('id_row'));
            $fields['id_row'] = (int)Tools::getValue('id_row', $row->id);
            $fields['active'] = Tools::getValue('active', $row->active);
            $fields['id_ttmegamenu'] = Tools::getValue('id_ttmegamenu', $row->id_ttmegamenu);
            $fields['class'] = Tools::getValue('class', $row->class);
        } else {
            $row = new TtMegamenuRowClass();
            $fields['active'] = Tools::getValue('active', 1);
            $fields['id_ttmegamenu'] = Tools::getValue('id_ttmegamenu', 1);
...
//        $languages = Language::getLanguages(false);
        if (Tools::isSubmit('id_column')) {
            $col = new TtMegamenuColumnClass((int)Tools::getValue('id_column'));
    ...
    public function renderBuildMenu()
    {
        ...
        $this
            ...
            ->controller
            ->addCSS($this->_path . 'views/css/back.css');
        $id_ttmegamenu = Tools::getValue('id_ttmegamenu', 1);
        $info_rows = $this->getRowInfo($id_ttmegamenu);

        foreach ($info_rows as $key => $info_row) {
            $info_rows[$key]['status'] = $this->displayStatusRow($id_ttmegamenu, $info_row['id_row'], $info_row['active']);
    ...
    public function getRowInfo($id_menu, $active = null)
    {
        ...
        $row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_row_shop mr
			WHERE mr.`id_shop` = ' . $id_shop . ' AND mr.id_ttmegamenu = ' . $id_menu . ($active ? ' AND mr.`active` = 1' : ' '));
    ...
    public function getColInfo($id_row, $active = null, $is_backend = true)
    {
        ...
        $cols_result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_column_shop mc
        ...
        if (is_array($col_infos) && count($col_infos) > 0) {
            foreach ($col_infos as $key => $col_info) {
                $sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					SELECT smi.*,smil.*
					FROM ' . _DB_PREFIX_ . 'ttmegamenu_item_shop smi
					LEFT JOIN ' . _DB_PREFIX_ . 'ttmegamenu_item_lang smil ON (smi.id_item = smil.id_item AND smi.id_shop = smil.id_shop)
                ...
                if (is_array($sub_menu_infos) && count($sub_menu_infos) > 0) {
                    foreach ($sub_menu_infos as $key1 => $sub_menu_info) {
                        $id_ttmegamenu = Tools::getValue('id_ttmegamenu', 1);
                        if (isset($id_ttmegamenu) && $id_ttmegamenu > 0 && $is_backend) {
                            $sub_menu_infos[$key1]['status'] = $this->displayStatusSubItem($id_ttmegamenu, $sub_menu_info['id_item'], $sub_menu_info['active']);
                        }
    ...
    public function getSubMenu($id_ttmegamenu)
    {
        $info_rows = array();
        if (is_array($this->getRowInfo($id_ttmegamenu, true)) && count($this->getRowInfo($id_ttmegamenu, true)) > 0) {
            $info_rows = $this->getRowInfo($id_ttmegamenu, true);
            foreach ($info_rows as $key => $info_row) {
    ...
    {
        $req = 'SELECT tt.`id_ttmegamenu` as id_ttmegamenu
				FROM `' . _DB_PREFIX_ . 'ttmegamenu` tt
				WHERE tt.`id_ttmegamenu` = ' . (int)$id;
...
//        $group_cat_result = array();
        if (!$this->isCached('ttmegamenu.tpl', $this->getCacheId('ttmegamenu'))) {
            $menu_obj = new TtMegamenuClass();
...
//            $languages = Language::getLanguages();
            ...
            foreach ($menus as $menu) {
                if ($menu['type_link'] == 0 && $menu['dropdown'] == 1) {
                    ...
                    $sub_menu = $this->getSubMenu($menu['id_ttmegamenu']);
            ...
            return ['menus' => $new_menus, 'icon_path' => $this->_path . 'views/img/icons/', '_path' => $this->_path , 'hookName' => $hookName ,'configuration' => $configuration];
        }
    }
    ...
    public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
    {
        ...
        if (!empty($is_intersected)) {
            ...
            $this->respMenu .= '<a class="ttinnermenu" href="' . $cat_link . '">';
            if ($type_icon == 1 && $icon != '') {
            ...
            if ($sub_title != '') {
                $this->respMenu .= '<div class="menu-subtitle">' . $sub_title . '</div>';
            }
            $this->respMenu .= '</a>';
        }
    ...
    public function hookActionShopDataDuplication($params)
    {
			...
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_shop (`id_ttmegamenu`, `id_shop`, `type_link`, `dropdown`, `type_icon`, `icon`, `align_sub`, `width_sub`, `class`, `active`)
			SELECT id_ttmegamenu, ' . (int)$params['new_id_shop'] . ', type_link, dropdown, type_icon, icon, align_sub, width_sub, class, active
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_lang (`id_ttmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_ttmegamenu, ' . (int)$params['new_id_shop'] . ', id_lang, title, link, subtitle
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_lang
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_row_shop (`id_row`, `id_ttmegamenu`, `id_shop`, `class`, `active`)
			SELECT id_row, id_ttmegamenu, ' . (int)$params['new_id_shop'] . ', class, active
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_row_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_column_shop (`id_column`, `id_row`, `id_shop`, `width`, `class`, `active`)
			SELECT id_column, id_row, ' . (int)$params['new_id_shop'] . ', width, class, active
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_column_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_item_shop (`id_item`, `id_column`, `id_shop`, `type_link`, `type_item`, `id_product`, `active`)
			SELECT id_item, id_column, ' . (int)$params['new_id_shop'] . ', type_link, type_item, id_product, active
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_item_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_item_lang (`id_item`, `id_shop`, `id_lang`, `title`, `link`, `text`)
			SELECT id_item, ' . (int)$params['new_id_shop'] . ', id_lang, title, link, text
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_item_lang
			WHERE id_shop = ' . (int)$params['old_id_shop']);
    }
    public function hookActionObjectLanguageAddAfter($params)
    {
            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'ttmegamenu_lang (`id_ttmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_ttmegamenu, id_shop, ' . (int)$params['object']->id . ', title, link, subtitle
			FROM ' . _DB_PREFIX_ . 'ttmegamenu_lang
