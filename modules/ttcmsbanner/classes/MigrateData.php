...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmsbannerinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmsbannerinfo_lang` il ON il.`id_ttcmsbannerinfo` = i.`id_ttcmsbannerinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsBanners = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmsbannerinfo();
        $info->text = reset($ttcmsBanners);
        $return &= $info->add();

        if (count($ttcmsBanners) > 1) {
            foreach ($ttcmsBanners as $key => $text) {
