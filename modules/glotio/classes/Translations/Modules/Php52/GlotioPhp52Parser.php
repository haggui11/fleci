...
require_once _PS_MODULE_DIR_ . 'glotio/vendor/included-vendors/nikic-parser-0-9/bootstrap.php';

/**
 * Implements PHP 5.2 compatible PHP parser (nikic/php-parser library version 0.9, supports parsing PHP 5.5 and older)
 */
class GlotioPhp52Parser implements GlotioPhpParser
{
    /**
    ...
    public function parsePhpFileToFindObjectModelClasses($phpFile)
    {
        ...
        $nodeVisitor = new GlotioPhp52NodeVisitor();
    ...
    /**
     * @param GlotioPhp52ParsedClassModel $parsedClassModel
    ...
    public function getGlotioModuleModel($parsedClassModel, $module_name)
    {
        ...
        $modelName  = Tools::toCamelCase($parsedClassModel->classname . $tablename, true);
     ...
     * @param PHPParser_Node_Stmt_PropertyProperty $definition_property
     * @return string|null
     */
    private function getTableNameFromDefinitionProperty($definition_property)
    {
        ...
        if ($definition_property && isset($definition_property->default->items)) {
            foreach ($definition_property->default->items as $item) {
                /** @var PHPParser_Node_Expr_ArrayItem $item */
