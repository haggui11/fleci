...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmstestimonialinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmstestimonialinfo_lang` il ON il.`id_ttcmstestimonialinfo` = i.`id_ttcmstestimonialinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsTestimonials = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmstestimonialinfo();
        $info->text = reset($ttcmsTestimonials);
        $return &= $info->add();

        if (count($ttcmsTestimonials) > 1) {
            foreach ($ttcmsTestimonials as $key => $text) {
