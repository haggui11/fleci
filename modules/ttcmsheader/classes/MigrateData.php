...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmsheaderinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmsheaderinfo_lang` il ON il.`id_ttcmsheaderinfo` = i.`id_ttcmsheaderinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsHeader = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmsheaderinfo();
        $info->text = reset($ttcmsHeader);
        $return &= $info->add();

        if (count($ttcmsHeader) > 1) {
            foreach ($ttcmsHeader as $key => $text) {
