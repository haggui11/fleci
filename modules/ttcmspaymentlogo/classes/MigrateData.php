...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmspaymentlogoinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmspaymentlogoinfo_lang` il ON il.`id_ttcmspaymentlogoinfo` = i.`id_ttcmspaymentlogoinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsPaymentlogo = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmspaymentlogoinfo();
        $info->text = reset($ttcmsPaymentlogo);
        $return &= $info->add();

        if (count($ttcmsPaymentlogo) > 1) {
            foreach ($ttcmsPaymentlogo as $key => $text) {
