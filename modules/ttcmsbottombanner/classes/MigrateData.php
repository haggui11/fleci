...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmsbottombannerinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmsbottombannerinfo_lang` il ON il.`id_ttcmsbottombannerinfo` = i.`id_ttcmsbottombannerinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsBottombanners = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmsbottombannerinfo();
        $info->text = reset($ttcmsBottombanners);
        $return &= $info->add();

        if (count($ttcmsBottombanners) > 1) {
            foreach ($ttcmsBottombanners as $key => $text) {
